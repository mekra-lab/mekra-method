...
Public repositories use Apache-2.0 for files unless otherwise noted. All files under `templates/`, including its README, are provided under [CC0-1.0](../templates/LICENSE) so they can be copied, adapted, and incorporated into a target repository. Using these templates does not require Mekra Method attribution or an adoption record. This exception does not extend to documents or experimental scaffolds outside `templates/`.
